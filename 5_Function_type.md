...
callSomeFunction()은 범용 함수이므로 첫 번째 매개변수에 어떤 함수를 넘기든 관계없이 해당 함수에 두 번째 매개변수를 넘겨 실행한 결과를 반환한다. 다시 말할해 <b>함수를 실행하지 않고, 단지 함수를 가리키는 포인터에 접근하기만 할 때는 괄호를 쓰면 안된다.</b> 함수가 함수를 반환하게 만들 수도 있으며 이런 패턴은 매우 유용하다 예를 들어 객체러 이루어진 배열이 있고 이 배열을 임의의 객체 프로퍼티를 기준으로 정렬하고 싶다고 가정하자 배열의 sort()매서드에 넘길 비교 함수는 매개변수를 두 개 밖에 받지 않아서 기준이 될 프로퍼티를 어떻게 식별 할 지 지정할 방법이 없다. 이 문제는 다음과 같이 프로퍼티 이름에 따라 비교 함수를 생성해 반환하는 함수를 만들어 해결 할 수 있다.  
<pre>
