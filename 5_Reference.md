...
+ g - 전역 모드를 지정한다 이플래그를 저정하면 문자열에서 패턴을 찾는 즉시 종료하지 않고 문자열 전체에서 동작한다.
+ i - 대소문자 비구분 모드를 지정한다. 이 플래그를 지정하면 패턴을 찾을 때 대소문자를 구분하지 않는다.
+ m - 여러 줄 모드를 지정한다 이 플래그를 지정하면 텍스트의 줄 끝에 도달 해도 멈추지 않고 계속 패턴을 찾는다.
정규 표현식은 다음과 같이 플래그에 따라 결과가 달라진다.
<pre>
/ *
  * 모든 값에 'at'에 일치
  * /
...
/ *
  * "bat"이나 'cat'중 처음 나온 것과 일치, 대소문자 구분 없음
  * /
var pattern2 = /[dc]at/i;

/ *
  * "at"으로 끝나는 세 글자에 모두 일치, 대수문자 구분없음
  * /
var pattern3 = /.at/gi;
</pre>
다른 언어의 정규 표현식과 마찬가지로 '메타문자'를 패턴에 쓸 때에는 반드시 이스케이프 해아 한다. 메타문자에는 정해진 쓰임새가 있으므로 문자열에서 메타문자에 해당하는 문자를 찾을 때는 역슬레시를 써서 이스케이프 해야 한다.  
<pre>
/ *
  * "bat"이나 'cat'중 처음 나온 것과 일치, 대소문자 구분 없음
  * /
var pattern1 = /[dc]at/i;

/ *
  * 처음나온 "[dc]at"에 일치, 대소문자 구분없음
  * /
var pattern2 = \/[dc\at\i;

/ *
  * "at"로 끝나는 세 글자에 모두 일치, 대소문자 구분없음
  * /
var pattern3 = /.at/gi;
...
생성자의 매개 변수는 둘 다 문자열이기때문에 RegExp생성자에 리터럴을 넘기면 안됀다. 필요하다면 \을 하나 더 추가하는 방식으로 해야 한다.  
