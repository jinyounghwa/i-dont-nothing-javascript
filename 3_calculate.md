...
일치/불일치 연산자는 동일/비동일 연ㅅ나자와 같은 일을 하지만 피연산자의 타입을 변환하지 않고 있는 그대로 비교한다.
일치 연산자는 피연산자의 타입을 변환하지 않아도 같을 때에만 true를 반환한다.
<pre>
var result1 = ("55" == 55) // 변환한 후 비교하면 일치하므로 true
var result2 = ("55" === 55) // 데이터 타입이 다르므로 일치하지 않아서 false
</pre>
불일치 연산자는 피연산자를 변환하지 않은 상태에서 일치하지 않을 때에만 true를 반환한다.
<pre>
var result1 = ("55" !== 55) // 변환한 후 비교하면 일치하므로 false
var result2 = ("55" !=== 55) // 데이터 타입이 다르므로 일치하지 않아서 true
</pre>
문자열 55는 숫자 55와 다른가에서 불일치 연산자 일때 맞는 이야기이다.
null == undefined 은 true이지만 타입은 다르기 때문에 null === undefined는 false이다.

3항 연산자
<pre>
var max = (num1 > mun2) ? num1 : num2;
</pre>
